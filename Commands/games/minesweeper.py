...
async def play(ctx, columns = None, rows = None, bombs = None):
	...
	if columns > 13 or rows > 13:
		return await ctx.send('由於Discord限制，列和行的限制是13')
	if columns < 1 or rows < 1 or bombs < 1:
		return await ctx.send('提供的數字不能為零或負...')
	if bombs + 1 > columns * rows:
		return await ctx.send(':boom:**BOOM**, 你有更多的炸彈，而不是網格上的空格，或者你試圖製作所有空間炸彈！')
	...
	embed = Embed(title='\U0001F642 踩地雷 \U0001F635', color=0xC0C0C0)
	embed.add_field(name='列:', value=columns, inline=True)
	embed.add_field(name='行:', value=rows, inline=True)
	embed.add_field(name='總數:', value=columns * rows, inline=True)
	embed.add_field(name='\U0001F4A3 數:', value=bombs, inline=True)
	embed.add_field(name='\U0001F4A3 百分比:', value=f'{percentage}%', inline=True)
	embed.add_field(name='要求者:', value=ctx.author.display_name, inline=True)
